...
:set GlobalVarTest do={
    ...
    :set res [$RunTestCase $res $GetGlobalVar "testVarStr" "nothing" "nothing" "helloMikrotik" "Set and get string value"]

    # SetGlobalVar & GetGlobalVar (String with Double Quotes)
    $SetGlobalVar "testVarQuotes" ("text \"with\" quotes")
    :set res [$RunTestCase $res $GetGlobalVar "testVarQuotes" "nothing" "nothing" ("text \"with\" quotes") "Set and get string with internal double quotes"]

    # SetGlobalVar & GetGlobalVar (Integer)
    $SetGlobalVar "testVarInt" 12345
    :set res [$RunTestCase $res $GetGlobalVar "testVarInt" "nothing" "nothing" 12345 "Set and get integer value"]
    ...
    :set res [$RunTestCase $res $GetGlobalVar "testVarFloat" "nothing" "nothing" 15.65 "Set and get float number value"]

    # SetGlobalVar & GetGlobalVar (IP Address)
    $SetGlobalVar "testVarIp" 192.168.88.1
    :set res [$RunTestCase $res $GetGlobalVar "testVarIp" "nothing" "nothing" 192.168.88.1 "Set and get IP address value"]

    # SetGlobalVar & GetGlobalVar (IP Prefix / Subnet)
    $SetGlobalVar "testVarPrefix" 10.0.0.0/24
    :set res [$RunTestCase $res $GetGlobalVar "testVarPrefix" "nothing" "nothing" 10.0.0.0/24 "Set and get IP prefix value"]

    # SetGlobalVar & GetGlobalVar (Time)
    $SetGlobalVar "testVarTime" 01:15:30
    :set res [$RunTestCase $res $GetGlobalVar "testVarTime" "nothing" "nothing" 01:15:30 "Set and get time value"]
    ...
    :set res [$RunTestCase $res $GetGlobalVar "testVarEmpty" "nothing" "nothing" "" "Set and get empty string"]

    # Overwrite Existing Value
    $SetGlobalVar "testVarOverwrite" "first"
    $SetGlobalVar "testVarOverwrite" "second"
    :set res [$RunTestCase $res $GetGlobalVar "testVarOverwrite" "nothing" "nothing" "second" "Overwrite existing global variable"]

    # Change Value Type
    $SetGlobalVar "testVarType" "text"
    $SetGlobalVar "testVarType" 555
    :set res [$RunTestCase $res $GetGlobalVar "testVarType" "nothing" "nothing" 555 "Overwrite string with integer"]

    # Boolean False
    $SetGlobalVar "testVarFalse" false
    :set res [$RunTestCase $res $GetGlobalVar "testVarFalse" "nothing" "nothing" false "Set and get boolean false"]

    # Integer Zero
    $SetGlobalVar "testVarZero" 0
    :set res [$RunTestCase $res $GetGlobalVar "testVarZero" "nothing" "nothing" 0 "Set and get zero"]

    # Negative Integer
    $SetGlobalVar "testVarNegative" -123
    :set res [$RunTestCase $res $GetGlobalVar "testVarNegative" "nothing" "nothing" -123 "Set and get negative integer"]
    ...
    :set res [$RunTestCase $res $GetGlobalVar "testVarLong" "nothing" "nothing" $longString "Set and get long string"]

    # Special Characters
    $SetGlobalVar "testVarSpecial" ("\\/\$[]{}();,:|")
    :set res [$RunTestCase $res $GetGlobalVar "testVarSpecial" "nothing" "nothing" ("\\/\$[]{}();,:|") "Set and get special characters"]

    # Multiple Updates
    :for i from=1 to=100 do={
        $SetGlobalVar "testVarLoop" $i
    }
    :set res [$RunTestCase $res $GetGlobalVar "testVarLoop" "nothing" "nothing" 100 "Multiple sequential updates"]
    ...
    :set res [$RunTestCase $res $GetGlobalVar "testVarA" "nothing" "nothing" "AAA" "Variables are independent (A)"]
    :set res [$RunTestCase $res $GetGlobalVar "testVarB" "nothing" "nothing" "BBB" "Variables are independent (B)"]

    # Space
    $SetGlobalVar "testVarSpace" "Hello World"
    :set res [$RunTestCase $res $GetGlobalVar "testVarSpace" "nothing" "nothing" "Hello World" "String with spaces"]

    # Leading and trailing spaces
    $SetGlobalVar "testVarSpaces" "  Hello World  "
    :set res [$RunTestCase $res $GetGlobalVar "testVarSpaces" "nothing" "nothing" "  Hello World  " "Leading and trailing spaces"]

    # Tabs
    $SetGlobalVar "testVarTabs" ("A\tB\tC")
    :set res [$RunTestCase $res $GetGlobalVar "testVarTabs" "nothing" "nothing" ("A\tB\tC") "String with tabs"]

    # New lines
    $SetGlobalVar "testVarNewLines" ("Line1\nLine2\nLine3")
    :set res [$RunTestCase $res $GetGlobalVar "testVarNewLines" "nothing" "nothing" ("Line1\nLine2\nLine3") "String with new lines"]

    # Carriage return
    $SetGlobalVar "testVarCR" ("Line1\rLine2")
    :set res [$RunTestCase $res $GetGlobalVar "testVarCR" "nothing" "nothing" ("Line1\rLine2") "String with carriage return"]

    # Quotes
    $SetGlobalVar "testVarQuotes2" ("\"Hello\"")
    :set res [$RunTestCase $res $GetGlobalVar "testVarQuotes2" "nothing" "nothing" ("\"Hello\"") "Double quotes"]

    # Single quotes
    $SetGlobalVar "testVarSingleQuotes" "'Hello'"
    :set res [$RunTestCase $res $GetGlobalVar "testVarSingleQuotes" "nothing" "nothing" "'Hello'" "Single quotes"]

    # Backslashes
    $SetGlobalVar "testVarBackslash" ("\\server\\share\\dir")
    :set res [$RunTestCase $res $GetGlobalVar "testVarBackslash" "nothing" "nothing" ("\\server\\share\\dir") "Backslashes"]

    # Dollar sign
    $SetGlobalVar "testVarDollar" ("\$abc$123")
    :set res [$RunTestCase $res $GetGlobalVar "testVarDollar" "nothing" "nothing" ("\$abc$123") "Dollar sign"]

    # Percent signs
    $SetGlobalVar "testVarPercent" "100% complete"
    :set res [$RunTestCase $res $GetGlobalVar "testVarPercent" "nothing" "nothing" "100% complete" "Percent signs"]

    # URL characters
    $SetGlobalVar "testVarUrl" "https://example.com/test?a=1&b=2#fragment"
    :set res [$RunTestCase $res $GetGlobalVar "testVarUrl" "nothing" "nothing" "https://example.com/test?a=1&b=2#fragment" "URL"]

    # File path
    $SetGlobalVar "testVarPath" ("C:\\Program Files\\RouterOS\\test.txt")
    :set res [$RunTestCase $res $GetGlobalVar "testVarPath" "nothing" "nothing" ("C:\\Program Files\\RouterOS\\test.txt") "Windows path"]

    # Shell characters
    $SetGlobalVar "testVarShell" "&|;<>`(){}[]"
    :set res [$RunTestCase $res $GetGlobalVar "testVarShell" "nothing" "nothing" "&|;<>`(){}[]" "Shell metacharacters"]

    # Math symbols
    $SetGlobalVar "testVarMath" "+-*/=%^"
    :set res [$RunTestCase $res $GetGlobalVar "testVarMath" "nothing" "nothing" "+-*/=%^" "Math symbols"]

    # Punctuation
    $SetGlobalVar "testVarPunctuation" ".,:!?@#~"
    :set res [$RunTestCase $res $GetGlobalVar "testVarPunctuation" "nothing" "nothing" ".,:!?@#~" "Punctuation"]

    # Mixed special characters
    $SetGlobalVar "testVarMixed" ("\"%\\\$&;=+?<>[]{}()\n\r\t")
    :set res [$RunTestCase $res $GetGlobalVar "testVarMixed" "nothing" "nothing" ("\"%\\\$&;=+?<>[]{}()\n\r\t") "Mixed special characters"]

    # Empty string
    $SetGlobalVar "testVarEmpty2" ""
    :set res [$RunTestCase $res $GetGlobalVar "testVarEmpty2" "nothing" "nothing" "" "Empty string"]
    ...
    :set res [$RunTestCase $res $GetGlobalVar "testVarAllChars" "nothing" "nothing" $allChars "String containing all byte values (0-255)"]
